---
name: nextjs-v14-app-patterns
description: Load skill ini saat membuat atau memperbaiki fitur Next.js 14 App Router: routing, server components, client components, API routes, middleware, data fetching, caching, atau optimisasi performa. Berisi pola Next.js 14 App Router (sync params, cached-by-default, useFormState, React 18).
---

# Next.js 14 App Router Patterns — Company Standards

## Perbedaan Penting v14 vs v15

| Aspek | v14 (PAKAI ini) | v15 (JANGAN pakai) |
|-------|-----------------|---------------------|
| `params` | Object langsung | `Promise` (harus await) |
| `searchParams` | Object langsung | `Promise` (harus await) |
| fetch | **Cached by default** | Tidak cached by default |
| Form hook | `useFormState` dari `react-dom` | `useActionState` dari `react` |
| Pending state | `useFormStatus` (child component) | `useActionState` return `isPending` |
| React | 18 | 19 |

## DO / DON'T — Pola Kritis v14

### params — Object langsung (BUKAN Promise)

```tsx
// ✅ DO: akses params langsung
export default async function Page({ params }: { params: { id: string } }) {
  const user = await getUser(params.id);
}

// ❌ DON'T: jangan await params (itu v15/v16)
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; // SALAH di v14 — params bukan Promise
}
```

### Form — useFormState dari react-dom (BUKAN useActionState)

```tsx
// ✅ DO: useFormState + useFormStatus terpisah (v14 / React 18)
import { useFormState } from "react-dom";
import { useFormStatus } from "react-dom"; // HARUS di child component
const [state, formAction] = useFormState(action, initialState);

// ❌ DON'T: useActionState tidak ada di React 18
import { useActionState } from "react"; // TIDAK ADA — ini React 19 (v15+)
```

### Fetch — Cached by default

```tsx
// ✅ DO: fetch tanpa opsi = cached (default v14)
const res = await fetch(url);

// ✅ DO: opt-out cache eksplisit jika perlu data fresh
const res = await fetch(url, { cache: "no-store" });

// ❌ DON'T: jangan pakai "use cache" (itu v16)
"use cache"; // TIDAK ADA di v14
```
...
// ❌ DON'T: proxy.ts tidak ada di v14
...
// app/users/[id]/page.tsx — Dynamic page (v14: params langsung, BUKAN Promise)
...
export default async function UserDetailPage({
  ...
  const user = await getUser(params.id); // Akses langsung, TANPA await params
...
## Data Fetching & Caching (Next.js 14)

**Penting**: Di Next.js 14, `fetch` **DI-CACHE secara default** (berbeda dari v15 yang tidak cache). Untuk opt-out, gunakan `cache: "no-store"` atau `next: { revalidate: 0 }`.
...
// Tanpa opsi cache → DI-CACHE (default Next.js 14)
...
**v14**: Gunakan `useFormState` dari `react-dom` (BUKAN `useActionState`). Untuk pending state, gunakan `useFormStatus` di child component.
...
// components/SubmitButton.tsx — useFormStatus HARUS di child component
"use client";
...
```tsx
// Penggunaan di route handler (v14: params langsung, BUKAN Promise)
...
export async function GET(
  ...
  try {
    const user = await getUser(params.id); // Akses langsung
...
### Dynamic Route Handler (v14: params langsung)
...
export async function GET(
  ...
  const { id } = params; // Akses langsung di v14 (TANPA await)
...
**Perbedaan dengan Next.js 15**: Di v15, `params` berubah menjadi `Promise` — harus di-`await` sebelum digunakan.
...
// app/users/[id]/page.tsx — Dynamic metadata (v14: params langsung)
import type { Metadata } from "next";
import { getUser } from "@/lib/api/users";
