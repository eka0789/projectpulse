---
name: nextjs-v14-pages-patterns
description: Load skill ini saat membuat atau memperbaiki fitur Next.js 14 Pages Router: routing, data fetching (getServerSideProps/getStaticProps), API routes, middleware, atau konfigurasi project. Berisi pola Next.js 14 Pages Router (React 18).
---

# Next.js 14 Pages Router Patterns — Company Standards

## DO / DON'T — Pola Kritis Pages Router v14
...
import { useSearchParams } from "next/navigation"; // SALAH — ini App Router
```

### Data Fetching — getServerSideProps/getStaticProps (BUKAN Server Components)

```tsx
// ✅ DO: gunakan getServerSideProps untuk SSR
export const getServerSideProps: GetServerSideProps = async () => {
  const data = await fetch(url);
  return { props: { data: await data.json() } };
};

// ❌ DON'T: Server Components tidak ada di Pages Router
export default async function Page() { // SALAH — Pages Router tidak support async component
  const data = await fetch(url);
}
```
...
export const metadata = { ... }; // TIDAK ADA di Pages Router
```
