---
name: nextjs-v15-pages-patterns
description: Load skill ini saat membuat atau memperbaiki fitur Next.js 15 Pages Router: routing, data fetching (getServerSideProps/getStaticProps), API routes, middleware, atau konfigurasi project. Berisi pola Next.js 15 Pages Router (React 19).
---

# Next.js 15 Pages Router Patterns — Company Standards

## Catatan v15 untuk Pages Router

Pages Router di v15 **tidak mendapat fitur baru** (maintenance mode). Perubahan yang mempengaruhi Pages Router:

- **React 19** — `ref` sebagai prop (tanpa `forwardRef`), `use()` hook tersedia
- **fetch tidak cached by default** — mempengaruhi `fetch()` di `getServerSideProps`/`getStaticProps`
- `middleware.ts` masih digunakan (belum rename ke `proxy.ts`, itu baru di v16)

## DO / DON'T — Pola Kritis Pages Router v15
...
```tsx
// ✅ DO: eksplisit set cache di getServerSideProps/getStaticProps
...
### Metadata — next/head (BUKAN generateMetadata)

```tsx
// ✅ DO: next/head untuk SEO
import Head from "next/head";
<Head><title>Page Title</title></Head>

// ❌ DON'T: generateMetadata hanya di App Router
export async function generateMetadata() { ... } // TIDAK ADA di Pages Router
```
...
```tsx
// middleware.ts (root project — sama dengan App Router)
